...
class ScoreService
{
    ...
    public function getList($data)
    {
        ...
        $query = $this->score::select('score_id', 'user_id', 'join_id', 'order_id', 'score', 'score_comment', 'score_type', 'score_status', 'score_created_at', 'score_replay_status', 'score_replay_content');
    ...
    public function addScore($params)
    {
        ...
        try
        {
            ...
            //最新平均评分
            $stock = $this->score::where('join_id', '=', $params['join_id'])
                                 ->where('score_type', '=', $params['score_type'])->avg('score');
            $stock = round($stock, 1);
            ...
            switch ($params['score_type'])
            {
                case $this->score::SCORE_TYPE_A:
                    $this->attractions->where('attractions_id', '=', $params['join_id'])
                                      ->increment('attractions_evaluation');
                    $this->attractions->where('attractions_id', '=', $params['join_id'])
                                      ->increment('attractions_score_num');
                    $this->attractions->where('attractions_id', '=', $params['join_id'])
                                      ->update(['attractions_score' => $stock]);
                    break;
